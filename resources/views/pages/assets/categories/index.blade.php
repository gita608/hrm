...
@section('content')
	...
	<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-4">
		<div class="my-auto">
			<h2 class="mb-1 text-dark fw-bold">Asset Categories</h2>
			<p class="text-muted mb-0 fs-13">Manage asset categories and classifications</p>
		</div>
		<div class="d-flex align-items-center gap-2">
			<a href="{{ route('assets.categories.create') }}" class="btn btn-primary rounded-pill shadow-sm py-2 px-3">
				...
				<div class="card-body d-flex align-items-center justify-content-between p-4">
					<div class="d-flex align-items-center overflow-hidden">
						...
						<div class="ms-3 overflow-hidden">
							<p class="fs-13 fw-medium mb-1 text-muted text-truncate text-uppercase ls-1">Total Categories</p>
							<h4 class="mb-0 fw-bold text-dark">{{ $categories->count() }}</h4>
						</div>
					</div>
				</div>
			</div>
				...
				<div class="card-body d-flex align-items-center justify-content-between p-4">
					<div class="d-flex align-items-center overflow-hidden">
						...
						<div class="ms-3 overflow-hidden">
							...
							<h4 class="mb-0 fw-bold text-dark">{{ $categories->where('is_active', true)->count() }}</h4>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="card flex-fill border-0 shadow-sm rounded-4 overflow-hidden position-relative">
				<div class="card-body d-flex align-items-center justify-content-between p-4">
					<div class="d-flex align-items-center overflow-hidden">
						...
						<div class="ms-3 overflow-hidden">
							<p class="fs-13 fw-medium mb-1 text-muted text-truncate text-uppercase ls-1">Inactive</p>
							<h4 class="mb-0 fw-bold text-dark">{{ $categories->where('is_active', false)->count() }}</h4>
						</div>
					</div>
				</div>
			</div>
	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		<div class="card-header bg-transparent border-bottom border-light pt-3 pb-2 d-flex align-items-center justify-content-between flex-wrap gap-3">
			...
			<div class="d-flex align-items-center gap-2 flex-wrap">
				<form method="GET" action="{{ route('assets.categories.index') }}" class="d-flex gap-2 flex-wrap align-items-center">
					<div>
					...
					@if(request()->has('status'))
						<a href="{{ route('assets.categories.index') }}" class="btn btn-sm btn-light rounded-pill px-3 shadow-none">Clear</a>
					@endif
				</form>
			</div>
		</div>
				...
				<table class="table table-hover align-middle mb-0">
					<thead class="bg-light-50">
						<tr>
							...
							<th class="border-0 text-muted fs-12 fw-medium text-uppercase">Assets</th>
					...
					<tbody>
						@forelse($categories as $category)
							<tr class="border-bottom border-light">
								...
								<td class="text-muted fs-13">{{ $category->code ?? 'N/A' }}</td>
								<td class="text-muted fs-13">{{ Str::limit($category->description ?? 'N/A', 50) }}</td>
								<td>
									<span class="badge bg-light text-dark border rounded-pill">{{ $category->assets()->count() }} Assets</span>
								</td>
								<td>
								...
								<td class="pe-3 text-end">
									<div class="d-flex justify-content-end gap-2">
										<a href="{{ route('assets.categories.show', $category->id) }}" class="btn btn-sm btn-icon btn-light rounded-circle hover-bg-primary hover-text-white transition-all shadow-sm" data-bs-toggle="tooltip" title="View">
											<i class="ti ti-eye"></i>
										</a>
										<a href="{{ route('assets.categories.edit', $category->id) }}" class="btn btn-sm btn-icon btn-light rounded-circle hover-bg-primary hover-text-white transition-all shadow-sm" data-bs-toggle="tooltip" title="Edit">
								...
								<td colspan="7" class="text-center py-5">
									<div class="d-flex flex-column align-items-center justify-content-center">
										<div class="avatar avatar-xxl bg-light-50 rounded-circle mb-3 text-muted">
											<i class="ti ti-category-2 fs-30"></i>
										</div>
