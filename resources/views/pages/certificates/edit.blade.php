...
@section('content')
	...
	<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-4">
		<div class="my-auto">
			<h2 class="mb-1 text-dark fw-bold">Edit Certificate</h2>
			<p class="text-muted mb-0 fs-13">Update certificate details and status</p>
		</div>
		...
		<div class="card-body p-4">
			<form action="{{ route('certificates.update', $certificate->id) }}" method="POST" enctype="multipart/form-data">
				...
				<div class="row">
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="text" class="form-control rounded-3 border-light shadow-none" name="title" value="{{ old('title', $certificate->title) }}" placeholder="e.g., Bachelor of Science" required>
							@error('title')
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="text" class="form-control rounded-3 border-light shadow-none" name="certificate_number" value="{{ old('certificate_number', $certificate->certificate_number) }}" placeholder="e.g., CERT-2026-001" required>
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="text" class="form-control rounded-3 border-light shadow-none" name="issuing_authority" value="{{ old('issuing_authority', $certificate->issuing_authority) }}" placeholder="e.g., University Name, Training Institute">
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="date" class="form-control rounded-3 border-light shadow-none" name="issue_date" value="{{ old('issue_date', $certificate->issue_date?->format('Y-m-d')) }}">
							@error('issue_date')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="date" class="form-control rounded-3 border-light shadow-none" name="expiry_date" value="{{ old('expiry_date', $certificate->expiry_date?->format('Y-m-d')) }}">
							@error('expiry_date')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-6">
						<div class="mb-3">
							<label class="form-label text-muted fs-12 text-uppercase fw-medium">File</label>
							<input type="file" class="form-control rounded-3 border-light shadow-none" name="file">
							<small class="text-muted fs-11">Leave empty to keep current file. Max file size: 10MB</small>
							@if($certificate->file_path)
								<div class="mt-2">
									<a href="{{ asset('storage/' . $certificate->file_path) }}" target="_blank" class="btn btn-sm btn-outline-primary rounded-pill px-3 shadow-none">
					...
					<div class="col-md-12">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none" name="description" rows="3">{{ old('description', $certificate->description) }}</textarea>
							@error('description')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-12">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none" name="notes" rows="2">{{ old('notes', $certificate->notes) }}</textarea>
							@error('notes')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
				...
				<div class="d-flex justify-content-end gap-2 mt-4">
					...
					<button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm">Update Certificate</button>
				</div>
			</form>
		</div>
	</div>
