...
@section('content')
	...
	<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-4">
		<div class="my-auto">
			<h2 class="mb-1 text-dark fw-bold">Create Designation</h2>
			<p class="text-muted mb-0 fs-13">Add a new designation to a department</p>
		</div>
		...
		<div class="card-body p-4">
			<form action="{{ route('designations.store') }}" method="POST">
				...
				<div class="row">
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="text" class="form-control rounded-3 border-light shadow-none" name="name" value="{{ old('name') }}" required placeholder="e.g. Senior Developer">
							@error('name')
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="text" class="form-control rounded-3 border-light shadow-none" name="code" value="{{ old('code') }}" placeholder="e.g. DEV-SR">
							@error('code')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-12">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none" name="description" rows="3" placeholder="Enter designation description here...">{{ old('description') }}</textarea>
							@error('description')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
				...
				<div class="d-flex justify-content-end gap-2 mt-3">
					<a href="{{ route('designations.index') }}" class="btn btn-light rounded-pill px-4">Cancel</a>
					<button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm">Save Designation</button>
				</div>
			</form>
		</div>
	</div>
