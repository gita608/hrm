...
@section('content')
	...
	<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-4">
		<div class="my-auto">
			<h2 class="mb-1 text-dark fw-bold">Edit Interview Feedback</h2>
			<p class="text-muted mb-0 fs-13">Update feedback for an interview</p>
		</div>
		...
		<div class="card-body p-4">
			<form action="{{ route('interviews.feedback.update', $feedback->id) }}" method="POST">
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none @error('feedback') is-invalid @enderror" name="feedback" rows="4">{{ old('feedback', $feedback->feedback) }}</textarea>
							@error('feedback')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none @error('strengths') is-invalid @enderror" name="strengths" rows="3">{{ old('strengths', $feedback->strengths) }}</textarea>
							@error('strengths')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none @error('weaknesses') is-invalid @enderror" name="weaknesses" rows="3">{{ old('weaknesses', $feedback->weaknesses) }}</textarea>
							@error('weaknesses')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-12">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none @error('additional_notes') is-invalid @enderror" name="additional_notes" rows="3">{{ old('additional_notes', $feedback->additional_notes) }}</textarea>
				...
				<div class="d-flex justify-content-end gap-2 mt-4">
					...
					<button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm">Update Feedback</button>
				</div>
			</form>
		</div>
	</div>
