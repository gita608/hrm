		...
		<div class="card-body p-4">
			<form action="{{ route('schedule.store') }}" method="POST">
				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="mb-3">
							<label class="form-label text-muted fs-12 text-uppercase fw-medium">Start Time <small class="text-muted text-lowercase font-weight-normal">(Default from shift if empty)</small></label>
							<input type="time" class="form-control rounded-3 border-light shadow-none" name="start_time" id="start_time" value="{{ old('start_time') }}">
							@error('start_time')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-6">
						<div class="mb-3">
							<label class="form-label text-muted fs-12 text-uppercase fw-medium">End Time <small class="text-muted text-lowercase font-weight-normal">(Default from shift if empty)</small></label>
							<input type="time" class="form-control rounded-3 border-light shadow-none" name="end_time" id="end_time" value="{{ old('end_time') }}">
							@error('end_time')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-12">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none" name="notes" rows="3" placeholder="Additional notes...">{{ old('notes') }}</textarea>
							@error('notes')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
				...
				<div class="d-flex justify-content-end gap-2 mt-3">
					...
					<button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm">Save Schedule</button>
				</div>
			</form>
		</div>
	</div>
