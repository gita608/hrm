...
@section('content')
	...
	<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-4">
		<div class="my-auto">
			<h2 class="mb-1 text-dark fw-bold">Edit Holiday</h2>
			<p class="text-muted mb-0 fs-13">Update holiday request details</p>
		</div>
		...
		<div class="card-body p-4">
			<form action="{{ route('holidays.update', $holiday->id) }}" method="POST">
				...
				<div class="row">
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="text" class="form-control rounded-3 border-light shadow-none" name="name" value="{{ old('name', $holiday->name) }}" required>
							@error('name')
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="date" class="form-control rounded-3 border-light shadow-none" name="date" value="{{ old('date', $holiday->date->format('Y-m-d')) }}" required>
							@error('date')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<select class="form-select rounded-3 border-light shadow-none" name="type" required>
					...
					<div class="col-md-12">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none" name="description" rows="3" placeholder="Enter holiday description...">{{ old('description', $holiday->description) }}</textarea>
							@error('description')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
				...
				<div class="d-flex justify-content-end gap-2 mt-3">
					<a href="{{ route('holidays.index') }}" class="btn btn-light rounded-pill px-4">Cancel</a>
					<button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm">Update Holiday</button>
				</div>
			</form>
		</div>
	</div>
