...
@section('content')
	...
	<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-4">
		<div class="my-auto">
			<h2 class="mb-1 text-dark fw-bold">Edit Promotion</h2>
			<p class="text-muted mb-0 fs-13">Update promotion details</p>
		</div>
		...
		<div class="card-body p-4">
			<form action="{{ route('promotions.update', $promotion->id) }}" method="POST">
				...
				<div class="row">
						...
						<div class="mb-3">
							...
							<select class="form-select rounded-3 border-light shadow-none @error('employee_id') is-invalid @enderror" name="employee_id" required>
								...
								@foreach($employees as $employee)
									<option value="{{ $employee->id }}" {{ old('employee_id', $promotion->employee_id) == $employee->id ? 'selected' : '' }}>{{ $employee->name }}</option>
								@endforeach
							</select>
							@error('employee_id')
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<div class="input-group">
								...
								<input type="date" class="form-control rounded-end-3 border-light shadow-none @error('promotion_date') is-invalid @enderror" name="promotion_date" value="{{ old('promotion_date', $promotion->promotion_date->format('Y-m-d')) }}" required>
							</div>
										...
										<div class="mb-3 mb-md-0">
											...
											<select class="form-select rounded-3 border-light shadow-none @error('from_department_id') is-invalid @enderror" name="from_department_id">
												...
												@foreach($departments as $department)
													<option value="{{ $department->id }}" {{ old('from_department_id', $promotion->from_department_id) == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
												@endforeach
											</select>
									...
									<div class="col-md-6">
										<div class="mb-0">
											...
											<select class="form-select rounded-3 border-light shadow-none @error('to_department_id') is-invalid @enderror" name="to_department_id">
												<option value="">Select Department</option>
												@foreach($departments as $department)
													<option value="{{ $department->id }}" {{ old('to_department_id', $promotion->to_department_id) == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
												@endforeach
											</select>
										...
										<div class="mb-3 mb-md-0">
											...
											<select class="form-select rounded-3 border-light shadow-none @error('from_designation_id') is-invalid @enderror" name="from_designation_id">
												...
												@foreach($designations as $designation)
													<option value="{{ $designation->id }}" {{ old('from_designation_id', $promotion->from_designation_id) == $designation->id ? 'selected' : '' }}>{{ $designation->name }}</option>
												@endforeach
											</select>
									...
									<div class="col-md-6">
										<div class="mb-0">
											...
											<select class="form-select rounded-3 border-light shadow-none @error('to_designation_id') is-invalid @enderror" name="to_designation_id">
												<option value="">Select Designation</option>
												@foreach($designations as $designation)
													<option value="{{ $designation->id }}" {{ old('to_designation_id', $promotion->to_designation_id) == $designation->id ? 'selected' : '' }}>{{ $designation->name }}</option>
												@endforeach
											</select>
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<div class="input-group">
								...
								<input type="number" step="0.01" class="form-control rounded-end-3 border-light shadow-none @error('salary') is-invalid @enderror" name="salary" value="{{ old('salary', $promotion->salary) }}">
							</div>
							@error('salary')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
					...
					<div class="col-md-12">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none @error('notes') is-invalid @enderror" name="notes" rows="3">{{ old('notes', $promotion->notes) }}</textarea>
							@error('notes')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
				...
				<div class="d-flex justify-content-end gap-2 mt-4">
					...
					<button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm">Update Promotion</button>
				</div>
			</form>
		</div>
	</div>
