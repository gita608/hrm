...
@section('content')
	...
	<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-4">
		<div class="my-auto">
			<h2 class="mb-1 text-dark fw-bold">Create Overtime</h2>
			<p class="text-muted mb-0 fs-13">Submit a new overtime request</p>
		</div>
		...
		<div class="card-body p-4">
			<form action="{{ route('overtime.store') }}" method="POST">
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="mb-3">
							...
							<textarea class="form-control rounded-3 border-light shadow-none" name="reason" rows="3" placeholder="Enter reason for overtime...">{{ old('reason') }}</textarea>
							@error('reason')
								<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
				...
				<div class="d-flex justify-content-end gap-2 mt-3">
					...
					<button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm">Save Overtime</button>
				</div>
			</form>
		</div>
	</div>
